...
def query_process_data(node:str, client: object, 
                       start: str, end: str, 
                       interval: str, value: str, 
                       time_list: list, labels: dict) -> dict:
    json_data = {}
    try:
        node_data = query_node_data(node, client, start, end, 
                                    interval, value, labels)
        if node_data:
            json_data = process_node_data(node, node_data, value, 
                                          time_list, labels)
    except:
        logging.error(f"Failed to query and process data of node: {node}, \
...
def query_node_data(node:str, client: object, start: str, end: str, 
                    interval: str, value: str, labels: dict) -> dict:
    node_data = {}
    try:
...
def query_reading(client: object, node: str, measurement: str, label: str, 
    ...
    try:
        ...
        logging.error(f"Failed to query {measurement} of node: {node}, \
                        time range: {start} - {end}, interval: {interval}, \
                        value: {value}")
    return reading
...
def query_job_list(client: object, node: str, 
    ...
    job_list = []
    try:
        query_sql = "SELECT DISTINCT(JobList) FROM NodeJobs WHERE NodeId='" \
                    + node + "' AND time >= '" + start + "' AND time < '" \
                    + end + "' GROUP BY *, time(" + interval + ") SLIMIT 1"
